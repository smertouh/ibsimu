...
void simu( int argc, char **argv, double Vpuller, double Vdump, string current_working_dir )
{
    ...
    //import previous scharge
    std::ifstream is_SCfield( sbase+"/scharge.dat" );
    MeshScalarField scharge( geom );
    MeshScalarField scharge_ave( geom );
    if( is_SCfield.good() ){
    	/*MeshScalarField *p1 = &scharge;
    	MeshScalarField *p2 = &scharge_ave;
    	MeshScalarField *mesh_SCfield = new MeshScalarField(is_SCfield);
		p1= mesh_SCfield;
		p2 = mesh_SCfield;
		*/
		scharge( is_SCfield);
    	scharge_ave( is_SCfield);
    	ibsimu.message(1) <<"old rho"<<"\n";
		
		}else{
		ibsimu.message(1) <<"new rho"<<"\n";
		//MeshScalarField scharge( geom );
		//MeshScalarField scharge_ave( geom );
		}
    ...
    for( size_t i = 0; i < imax; i++ ) { //15(1)
    	...
    	geomplotter.plot_png( sbase+"/"+sfile+"/geom_zx"+to_string(i)+".png" );
    	geomplotter.set_view( VIEW_ZY, -1 );
    	geomplotter.plot_png( sbase+"/"+sfile+ "/geom_zy"+to_string(i)+".png" );
    	geomplotter.set_view_si( VIEW_XY, 5.0e-3 );
    	geomplotter.plot_png( sbase+"/"+sfile+ "/geom_xy"+to_string(i)+".png" );
    	}
    //save yyp profile
    ...
    scharge.save(sbase +"/scharge.dat");
}
